...
def add_course():
    ...
    if session['lg']=="lin":
        if request.method=="POST":
            ...
            db=Db()
            a=db.selectOne("select * from course where course_name='"+course_name+"' and dep_id='"+department+"'")
            if a is not None:
                return '<script>alert("Already exists");window.location="/add_course"</script>'
            else:
...
def add_department():
    ...
    if session['lg'] == "lin":
        if request.method=="POST":
           ...
           db = Db()
           b = db.selectOne("select * from department where dep_name='" + dn + "' ")
           if b is not None:
               return '<script>alert("Already exists");window.location="/add_department"</script>'
           else:
...
def approve_candidate ():
    ...
    if session['lg'] == "lin":
        db = Db()
        res = db.select("select * from candidate,student where candidate.student_id=student.student_id and `candidate`.`status`='approved' ")
...
def vote2(vid):
    ...
    if session['lg'] == "lin":
       db=Db()
       res=db.select("select * from candidate,student where candidate.student_id=student.student_id and `candidate`.`status`='approved' and post_id='" + vid + "'")
...
def view_live_results():
    ...
    if session['lg'] == "lin":
        if request.method=="POST":
            ...
            res1 = db.select("select * from election")
            res=db.select("SELECT  voting.*,candidate.*,post.*,student.*,election.*,COUNT(`voting`.`candidate_id`) as c,`election`.`status` AS es FROM voting,candidate,post,student,election WHERE candidate.candidate_id=voting.candidate_id AND candidate.post_id=post.post_id AND post.election_id=election.election_id AND candidate.student_id=student.student_id AND post.post_id='"+post+"' AND election.election_id='"+election+"' GROUP BY `candidate`.`candidate_id` ORDER BY c DESC")
            result=[]
            for i in res:
        ...
        if request.method == "POST":
            ...
            for i in res:
                if i['vote_id'] != '' and i['es'] == 'published':
                    result.append(i)
            if len(result) > 0:
                return render_template('candidate/view live results.html', data=result, data1=res1, data2=res2)
            else:
                return render_template('candidate/view live results.html', data1=res1, data2=res2)
        else:
            db = Db()
            res1 = db.select("select * from election")
            res2 = db.select("select * from post")
